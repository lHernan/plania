...
import { ChevronDown, Plus, Trash2, MapPin, Loader2, Check, X } from "lucide-react";
...
export function TripSwitcher() {
  ...
  const sheet = (
    <AnimatePresence>
      {isOpen && (
        <>
          {/* Backdrop — rendered in portal so z-index works globally */}
          <motion.div
            key="backdrop"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          <motion.div
            ...
            className="fixed inset-x-0 bottom-0 z-[210] bg-white dark:bg-slate-950 rounded-t-3xl flex flex-col"
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100 dark:border-slate-800 flex-shrink-0">
              <span className="text-sm font-black text-slate-900 dark:text-white tracking-tight">My Trips</span>
              <button
                onClick={() => { setIsOpen(false); setShowCreate(false); }}
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain px-3 py-2 space-y-1.5 min-h-0">
              {trips.length === 0 && !loading && (
                <div className="py-10 text-center">
                  <MapPin size={28} className="text-slate-300 mx-auto mb-3" />
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">No trips yet</p>
                </div>
              )}
              {trips.map((trip) => {
                ...
                return (
                  <div key={trip.id} className="relative group">
                    <button
                      onClick={() => { switchTrip(trip.id); setIsOpen(false); setShowCreate(false); }}
                    ...
                    >
                      <div className={`size-9 rounded-xl flex items-center justify-center flex-shrink-0 ${
                        isActive ? "bg-white/20 text-white" : "bg-white dark:bg-slate-800 text-slate-400"
                      }`}>
                        {isActive ? <Check size={18} /> : <MapPin size={18} />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm font-bold truncate ${isActive ? "text-white" : "text-slate-900 dark:text-white"}`}>
                          {trip.name}
                        </p>
                        <p className={`text-[10px] font-medium mt-0.5 ${isActive ? "text-white/70" : "text-slate-400"}`}>
            ...
            {/* Footer — Create form or button, always visible */}
  ...
  return (
    <div className="relative">
      {/* Trigger button — compact, always visible */}
      <button
        onClick={() => setIsOpen(true)}
        className="flex items-center gap-1.5 max-w-[180px] md:max-w-[220px]"
      >
        <div className="flex flex-col items-start min-w-0">
          <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 leading-none mb-0.5">Current Trip</span>
          <span className="text-sm font-black text-slate-900 dark:text-white truncate max-w-[140px] md:max-w-[180px] leading-tight">
      ...
      {/* Portal — renders outside any clipping parent */}
